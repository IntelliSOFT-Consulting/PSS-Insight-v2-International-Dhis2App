...
export default function useUpdateDictionary() {
  ...
  const updateDataElements = async (prevDictionary, currentDictionary) => {
    const dataElementNames = currentDictionary?.assessmentQuestions?.map(
      (question) => question.name
    );
    const prevDataElementNames = prevDictionary?.assessmentQuestions?.map(
      (question) => question.name
    );

    const dataElementsToAdd = dataElementNames?.filter(
      (name) => !prevDataElementNames?.includes(name)
    );
    let dataElementsToUpdate = dataElementNames?.filter((name) =>
      prevDataElementNames?.includes(name)
    );
    ...
    const newElements = dataElementsToAdd?.map((name) => {
      const question = currentDictionary.assessmentQuestions.find(
    ...
    if (dataElementsToUpdate?.length > 0) {
      ...
      const elements = dataElements?.dataElements?.map((element) => {
        const question = dataElementsToUpdate.find(
          (question) =>
            question.name === element.code ||
            currentDictionary?.indicatorCode === element.code
        );
        return {
          ...element,
          name: question?.name,
    ...
    const updatedDataElements = await engine.query({
      dataElements: {
        ...
        params: {
          fields:
            "id,name,optionSet,code,valueType,aggregationType,optionSet[id,name,options[id,name]]",
  ...
  const updateDictionary = async (prevDictionary, currentDictionary) => {
    try {
      ...
      const dataElements = await updateDataElements(
        prevDictionary,
        currentDictionary
      );
